...
//
//        /// Auto-advance timer: every 5 seconds, go to next index
//        private let autoScrollTimer = Timer.publish(every: 5.0, on: .main, in: .common).autoconnect()
//
//        var body: some View {
//            ZStack {
//                if !urlsFetched {
//                    // Show hidden WebView scraper until we get [URL]
//                    ScrapingWebView { urls in
//                        if urls.isEmpty {
//                            // If somehow no URLs were found, show an error
...
//                }
//                else {
//                    // Once we have the URL list, show either a loader, error, or the carousel:
...
//                                VideoPlayerView(
//                                    video: url,
//                                    player: viewModel.makePlayer(for: url)
//                                )
//                                .tag(idx)
//                                .ignoresSafeArea()
//                            }
...
//            selectedIndex = (selectedIndex + 1) % viewModel.videos.count
//        }
//
//        /// Go back to the previous video (wrap around to last index)
//        private func rewindIndex() {
//            guard !viewModel.videos.isEmpty else { return }
//            selectedIndex = (selectedIndex - 1 + viewModel.videos.count) % viewModel.videos.count
//        }
//
//        /// Reset state so that ScrapingWebViewHorizontal will reload /feed
//        private func reload() {
//            urlsFetched = false
//            viewModel.videos = []
...
//                    // Show a simple loader (or black screen) while we create AVPlayers.
//                    Color.black
//                        .ignoresSafeArea()
//                        .onAppear {
...
//                }
//                else {
//                    // MARK: 3) ALL PLAYERS READY → SHOW CAROUSEL
//                    TabView(selection: $selectedIndex) {
//                        ForEach(Array(viewModel.videos.enumerated()), id: \.offset) { idx, url in
//                            VideoPlayerView(
//                                video: url,
...
//                    }
//                    .tabViewStyle(PageTabViewStyle(indexDisplayMode: .never))
//                    // Whenever selectedIndex changes (swipe or programmatic), pause all but index, play index
...
//        // Clear any old players first
//        players.removeAll()
//
//        // Build exactly one AVPlayer per video URL
//        players = viewModel.videos.map { AVPlayer(url: $0) }
//
//        // Start playing index 0 immediately
//        playVideo(at: 0)
//    }
//
//    /// Pause all players except the one at `index`, which we seek to zero & play.
...
//
//    // ────────────────────────────────────────────────────────────────────────────
//    // MARK: – Manual + “Video Completed” Navigation
//    // ────────────────────────────────────────────────────────────────────────────
//
//    /// Advance to the next video (wrap around to zero)
//    private func advanceIndex() {
